...
@section('title', $pageTitle . ' | Career Gyan')
...
<style>
    ...
    .stat-num {
        ...
        background: linear-gradient(to right, #fff, var(--st-secondary));
    ...
    .path-card:hover {
        ...
        box-shadow: 0 30px 60px rgba(var(--st-primary-rgb),.3);
        border-color: rgba(var(--st-primary-rgb),.5);
    ...
    .path-icon {
        ...
        width: 42px; height: 42px; background: rgba(var(--st-primary-rgb),.85);
    ...
    .view-details {
        ...
        color: var(--st-primary); font-weight: 700; font-size: 14.5px; transition: all .3s;
    }
    .path-card:hover .view-details { gap: 13px; color: var(--st-secondary); }
    ...
    .pagination .active .page-link { background: var(--st-primary); border-color: var(--st-primary); color: #fff; }
    .pagination { justify-content: center; margin-bottom: 60px; }

    @media(max-width:768px) {
        .st-hero { padding: 80px 0 80px; border-radius: 0 0 40px 40px; }
...
<script>
    // Add RGB variables for the theme primary color dynamically for rgba usage
    document.addEventListener('DOMContentLoaded', function() {
        const primary = '{{ $theme['primary'] }}';
        // Basic hex to rgb converter (assuming #RRGGBB format)
        let r = parseInt(primary.slice(1, 3), 16),
            g = parseInt(primary.slice(3, 5), 16),
            b = parseInt(primary.slice(5, 7), 16);
        if(!isNaN(r) && !isNaN(g) && !isNaN(b)) {
            document.documentElement.style.setProperty('--st-primary-rgb', `${r}, ${g}, ${b}`);
        } else {
             document.documentElement.style.setProperty('--st-primary-rgb', `139, 92, 246`); // Fallback
        }
    });
</script>
@endsection
...
<div class="container">
    ...
    <h2 style="font-family:'Sora';font-size:clamp(20px,3vw,30px);font-weight:800;margin-bottom:40px;text-align:center;color:#fff;">
        Explore {{ $pageTitle }} Career Paths
    </h2>
    ...
    <div class="career-path-grid">
        ...
        <a href="{{ route('career.detail.page', $path->slug) }}" class="path-card">
            <div class="path-img-wrapper">
                ...
                >
                <div class="path-icon">
                    <i class="fa-solid {{ $path->icon ?? 'fa-briefcase' }}"></i>
                </div>
            ...
            <div class="path-body">
                <span class="salary-badge">{{ $path->salary_range ?? 'Varies' }}</span>
                <p class="path-desc">{{ Str::limit($path->description, 100) }}</p>
    ...
    <div style="text-align:center;margin-bottom:80px;">
        <a href="{{ route('explore.index') }}"
           style="display:inline-block;padding:18px 52px;font-size:16px;border-radius:30px;
                  background:{{ $theme['button_gradient'] }};color:#fff;font-weight:700;
                  box-shadow:0 12px 30px rgba(var(--st-primary-rgb),.4);text-decoration:none;">
